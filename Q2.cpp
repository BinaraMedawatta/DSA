#include <stdio.h>
#define MAX 4										//	Max is the total number of chairs in the barber shop (Including the barber room) 
	...
	do{
		...
		switch(cho){
			...
			case 3 :
				b = leave();
				if(b==0){
					printf("Barber leaves");		//	Barber leaves the shop -> (*1)
					cho = 3;
				}
				else if(b==1){
					printf("Barber can't leave, there is/are %d client(s) left", count);
					cho = 0;
					continue;
				}
		}
...
int leave(){
	if(count==0){									//	(*1) The barber can leave if and only if the count is zero. ie. there are no any 
		return 0;									//	clients left in the shop. 
	}
	else{
		return 1;									//	Else he cannot leave.
	}
}
